<script>
    ...
    $j.ajax({
        ...
        success: function(data) {
            ...
            $j.each(data.data, function(key, value) {
                options += "<option value='" + value.id + "'>" + value.vessel_name + "</option>";
            });
            $j("#add-vessel-id").html(options);
        },
        error: function(xhr) {
    ...
    $j('#accommodationForm').submit(function(event) {
        ...
        const vesselId = $j('#add-vessel-id').val();
        ...
        axios.post(event.target.action, formData, {
            ...
            .then(response => {
                ...
                form.reset();
                dttb.ajax.reload();
                $j('#addAccommodationModal').modal('hide');
    ...
    $j(document).on('click', '.edit', function(event) {
        ...
        $j.ajax({
            ...
            type: "GET",
            success: function(data) {
                $j('#edit-accommodation-form').attr('action', "/dashboard/settings/accomodations/" + accommodation_id);
                var options;
                $j.each(data.vessels, function(key, value) {
                    options += "<option value='" + value.id + "'>" + value.vessel_name + "</option>";
                });
                $j('#edit-vessel-id').empty().append(options).val(data.accommodation.vessel_id).trigger('change');
                $j('input[name="edit_accommodation_name"]').val(data.accommodation.accommodation_name);
                var imageUrl = data.image_url;
                // dropify.setValue(imageUrl);
                if (imageUrl) {
                    var dropifyWrapper = $('.dropify-wrapper');
                    dropifyWrapper.find('.dropify-preview .dropify-render img').attr('src', imageUrl);
                    $('input[name="image"]').attr('data-default-file', imageUrl);
                    $('.dropify-edit').dropify('destroy');
                    $('.dropify-edit').dropify();
                }
                $j('input[name="accommodation_id"]').val(data.accommodation.id);
                $j('input[name="edit_cottage_qy"]').val(data.accommodation.cottage_qy);
                $j('#edit-accommodation-modal').modal('show');
                // console.log($('.dropify').data('dropify'));
                // console.log($('.dropify-render img').get(0).complete);
            },
    ...
    $j('#edit-accommodation-form').on('submit', function(e) {
        e.preventDefault();
        var accommodation_id = $j('input[name="accommodation_id"]').val();
        ...
        $j.ajax({
            ...
            success: function(data) {
                $j('#edit-accommodation-modal').modal('hide');
                $j('#accommodations-table').DataTable().ajax.reload();
                table.draw();
    ...
    $j('#accommodations-table').on('click', '.delete', function() {
        var url = $j(this).data('url');
        var accommodationId = $j(this).data('id');
